...
import { logger, otpGenerate, sendMail } from "../helpers/index.js"
...
export const forgetPassword = async(req, res, next) =>{
    try {
        ...
        const newToken = jwt.sign(payload, process.env.FORGET_KEY, {expiresIn: process.env.FORGET_TIME})

        const link = `http://localhost:4000/auth/updateForget/${newToken}`
        await sendMail(email, "UPDATE", `this is your passwor for link: ${link}`)
        // await Users.updateOne(
        //     {email},
        //     {password: newPassword}
        // )
        return res.status(200).send({
            message: "new password link send your gmail"
        })
    } catch (error) {
        logger.error(error)
        next(error)
...
export const updateForget = async(req, res, next) =>{
    try {
        const {password} = req.body
        const {token} = req.params

        const decode = jwt.verify(token, process.env.FORGET_KEY)
        const {email} = decode
        const user = await Users.findOne({email})        
        if(!user){
            return res.status(404).send({
